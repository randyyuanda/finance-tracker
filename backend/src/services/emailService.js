...
exports.sendOtpEmail = async (to, otp) => {
  try {
    const info = await transporter.sendMail({
      from: process.env.SMTP_FROM || 'BuxBux <noreply@buxbux.com>',
      to,
      ...
      html: `
        <div style="font-family: Arial, sans-serif; padding: 20px; border: 1px solid #eee; border-radius: 10px; max-width: 480px;">
          ...
          <h1 style="color: #1677ff; letter-spacing: 5px; font-size: 36px; background: #f0f5ff; padding: 10px 20px; display: inline-block; border-radius: 8px;">${otp}</h1>
